Feature: Checkout and Payment
  ...
  Scenario Outline: User enters valid shipping details and places order
    ...
    When they enter valid shipping details "<firstName>", "<lastName>", "<zipCode>" and click "Continue"
  ...
  Scenario Outline: User enters invalid shipping details
    Given the user is on the checkout page
    When they enter invalid shipping details "<firstName>", "<lastName>", "<zipCode>" and click "Continue"
  ...
  @boundary
  Scenario: User enters boundary value for zip code
    Given the user is on the checkout page
    When they enter valid shipping details "Alice", "Brown", "00001" and click "Continue"
    Then the order should be successfully placed
    And they should see a page with order summary (items, shipping address, payment details)
    And click on Finish to complete the order